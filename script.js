...
function updateUI() {
    ...
    document.getElementById('readingDisplay').textContent = state.data.length > 0 ? state.data[state.data.length - 1].value : 0;
    document.getElementById('statusDisplay').textContent = state.isRunning ? 'Running' : (state.time >= 100 ? 'Complete' : 'Ready');
...
function renderDiagram() {
    ...
    const liquidColor = state.method === 'cross' ? (state.isRunning ? '#cbd5e1' : '#e2e8f0') : '#bae6fd';
    const liquidPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    liquidPath.setAttribute('d', 'M 85 110 L 115 110 L 140 170 C 145 185 135 190 120 190 L 80 190 C 65 190 55 185 60 170 Z');
    ...
    const flaskPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    flaskPath.setAttribute('d', 'M 85 50 L 115 50 L 115 100 L 150 180 C 160 200 140 200 120 200 L 80 200 C 60 200 40 200 50 180 L 85 100 Z');
    ...
    if (state.method === 'gas') {
        const bungPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        ...
        tubePath.setAttribute('d', 'M 100 70 L 100 20 L 180 20 L 180 30 L 110 30 L 110 70 Z');
    ...
    if (state.method === 'cross') {
        ...
        eyePath.setAttribute('d', 'M 0 10 Q -5 5 -5 0 Q -5 -5 0 -10 Q 5 -5 5 0 Q 5 5 0 10 Z M 0 2 C -2 2 -3 1 -3 0 C -3 -1 -2 -2 0 -2 C 2 -2 3 -1 3 0 C 3 1 2 2 0 2 Z');
...
function renderChart() {
    ...
    ctx.strokeStyle = 'rgba(55, 210, 176, 0.16)';
    ctx.lineWidth = 1;
    for (let i = 0; i <= 10; i++) {
    ...
    ctx.strokeStyle = 'rgba(244, 248, 251, 0.75)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ...
    if (state.data.length > 0) {
        ctx.strokeStyle = '#37d2b0';
        ctx.lineWidth = 3;
        ctx.beginPath();

        state.data.forEach((point, index) => {
            const x = padding + (point.time / 100) * width;
            const y = chartCanvas.height - padding - ((point.value / maxValue) * height);
